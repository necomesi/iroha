...
/* ============================== Class : Iroha.SlidePaneView ============================== */
/**
 * @class 左右にスライドしてサブペインをひきだす UI
 * @extends Iroha.Observable
 */
Iroha.SlidePaneView = function() {
	/**
...
Iroha.ViewClass(Iroha.SlidePaneView).extend(Iroha.Observable);

$.extend(Iroha.SlidePaneView,
/** @lends Iroha.SlidePaneView */
{
	/**
	 ...
	 */
	CLASSNAME : {
		  'baseNode'     : 'iroha-slidepaneview'
		, 'mainPane'     : 'iroha-slidepaneview-mainpane'
		, 'subPane'      : 'iroha-slidepaneview-subpane'
		, 'selectedPane' : 'iroha-slidepaneview-subpane-selected'
	},
	 ...
	 */
	DATANAME : {
		  'scrollTop' : 'Iroha.SlidePaneView.scrollTop'
	},
...
$.extend(Iroha.SlidePaneView.prototype,
/** @lends Iroha.SlidePaneView.prototype */
{
	/**
	 * 初期化。
	 ...
	 * @type Number|Iroha.SlidePaneView
	 */
	...
	addPane : function(kind, pane, button) {
		...
		if (!$oldPane) {
			throw new Error('Iroha.SlidePaneView#addPane: 引数 "kind" は "main", "left", "right" のいずれかでなければなりません。');
...
//										!opened ? this.open($left ) : $right.is($shown) && this.close();  // <textarea> 内のフリックも反応する等の誤動作の除外が面倒
	...
	open : function(pane) {
		...
		var eventNS   = 'Iroha.SlidePaneView.toggle';
