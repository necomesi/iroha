/*! "iroha.redirect.js" | Iroha - Necomesi JSLib : URL Redirector | by Necomesi Ltd. */
/* -------------------------------------------------------------------------- */
/**
 *    @fileoverview
 *       Iroha - Necomesi JSLib : URL Redirector
 *
 *    @version 3.01.20130927
 *
 *    @example
		...
		if (THISFILE.test(fname)) {
			...
			if (param.url) {
				...
				if (param.hide == 'yes') {
					...
					if (sheet) {
						sheet.insertRule
							? sheet.insertRule('html { display: none !important }', 0)             // modern Browsers
							: sheet.addRule && sheet.addRule('html', 'display: none !important');  // old Fxxing IEs
					}
				}
